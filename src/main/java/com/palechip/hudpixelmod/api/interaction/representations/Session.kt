...
class Session : UuidCallback {
    // the player this session belongs to, isn't included in the response but still relevant
    private var sessionOwner: String? = null

    val id: String? = null
    val replyGameType: String? = null
    private val players: JsonArray? = null
    private var playersArray: ArrayList<String>? = null
    val server: String? = null

    private var gameID: Int = 0 // saves the ID the mod uses for the game and not the database ID.
    private var gameTypeIntern: GameType? = null

    fun getGameID(): Int {
        if (this.gameID == 0) {
            // go through all configurations
            for (config in GameType.values()) {
                // if we find one with a matching database id
                if (config.modID == this.gameID) {
                    // save it
                    this.gameID = config.modID
                }
            }
        }
        return this.gameID
    ...
    fun getPlayers(): ArrayList<String>? {
        ...
        if (this.playersArray == null) {
            this.playersArray = ArrayList<String>()
            for (s in this.players!!) {
                if (s.isJsonPrimitive) {
                    this.playersArray!!.add(s.asString)
                }
            }
        }
    ...
    fun getSessionOwner(): String? {
        return this.sessionOwner
    }

    fun setSessionOwner(owner: UUID) {
        UuidHelper(owner.toString(), this)
    }

    fun setSessionOwner(owner: String) {
